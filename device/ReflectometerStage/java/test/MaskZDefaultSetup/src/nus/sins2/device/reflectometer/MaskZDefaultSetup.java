...
/**
 * Test Program for NUS.SINS2 Reflectometer MaskZ stage default setup
 * <p>
 * @author Carl Cork &lt;cwcork@lbl.gov&gt;
 */
public class MaskZDefaultSetup
{
  private static final Logger logger = Logger.getLogger(MaskZDefaultSetup.class.getName());
  private static final String DEFAULT_NODENAME = "/nus/sins2/device/reflectometer/stage";
  private static final double VENDOR_OFFSET = 30.00;
  ...
  public MaskZDefaultSetup(String nodeName)
...
//    Logger.getLogger("").setLevel(Level.ALL); // ALL levels
    ...
    try
    {
      ...
      test = new MaskZDefaultSetup(nodeName);

      // Describe setup
      System.out.println("MaskZDefaultSetup:");
  ...
  {
    System.out.println(
    "USAGE: java -jar MaskZDefaultSetup.jar "
    + "[<nodeName [String]>]");
  }
}
