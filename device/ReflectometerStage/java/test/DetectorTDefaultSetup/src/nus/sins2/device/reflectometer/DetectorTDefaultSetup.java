...
/**
 * Test Program for NUS.SINS2 Reflectometer Detector stage default setup
 * <p>
 * @author Carl Cork &lt;cwcork@lbl.gov&gt;
 */
public class DetectorTDefaultSetup
{
  private static final Logger logger = Logger.getLogger(DetectorTDefaultSetup.class.getName());
  private static final String DEFAULT_NODENAME = "/nus/sins2/device/reflectometer/stage";
  private static final double VENDOR_OFFSET = 7.79;
  ...
  public DetectorTDefaultSetup(String nodeName)
  throws IOException, InterruptedException
  {
    stage = new ReflectometerStage(nodeName);
    axis = stage.getDetectorT();
  }
...
//    Logger.getLogger("").setLevel(Level.ALL); // ALL levels
    ...
    try
    {
      ...
      test = new DetectorTDefaultSetup(nodeName);

      // Describe setup
      System.out.println("DetectorTDefaultSetup:");
      ...
      // Set position to vendor specified offset
      test.axis.setPosition(VENDOR_OFFSET);
  ...
  {
    System.out.println(
    "USAGE: java -jar DetectorTDefaultSetup.jar "
    + "[<nodeName [String]>]");
  }
}
