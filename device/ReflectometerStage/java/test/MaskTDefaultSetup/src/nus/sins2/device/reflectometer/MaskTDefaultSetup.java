...
/**
 * Test Program for NUS.SINS2 Reflectometer MaskT stage default setup
 * <p>
 * @author Carl Cork &lt;cwcork@lbl.gov&gt;
 */
public class MaskTDefaultSetup
{
  private static final Logger logger = Logger.getLogger(MaskTDefaultSetup.class.getName());
  private static final String DEFAULT_NODENAME = "/nus/sins2/device/reflectometer/stage";
  private static final double VENDOR_OFFSET = 148.47;
  ...
  public MaskTDefaultSetup(String nodeName)
...
//    Logger.getLogger("").setLevel(Level.ALL); // ALL levels
    ...
    try
    {
      ...
      test = new MaskTDefaultSetup(nodeName);

      // Describe setup
      System.out.println("MaskTDefaultSetup:");
  ...
  {
    System.out.println(
    "USAGE: java -jar MaskTDefaultSetup.jar "
    + "[<nodeName [String]>]");
  }
}
